...
uint16 zcl_event_loop( uint8 task_id, uint16 events )
{
  ...
  if ( events & SYS_EVENT_MSG )
  {
    ...
    while ( msgPtr != NULL )
    {
      ...
      {
        osal_msg_send( zcl_RegisteredMsgTaskID, msgPtr ); // send it to another task to process.
        dealloc = FALSE;
      }
      if ( dealloc ) // Release the memory
      {
        osal_msg_deallocate( msgPtr );
      }
      msgPtr = osal_msg_receive( zcl_TaskID ); // Next
    } 
          ...
          uint16 endClusterID, zclInHdlr_t pfnIncomingHdlr )
{
  ...
  if (  plugins == NULL ) // Find spot in list
  {
    plugins = pNewItem;
  }
  else
  {
    ...
    pLoop->next = pNewItem; // Put new item at end of list
  }
  return ( ZSuccess );
}
...
ZStatus_t zcl_registerAttrList( uint8 endpoint, uint8 numAttr, CONST zclAttrRec_t newAttrList[] )
{
  ...
  pNewItem = osal_mem_alloc( sizeof( zclAttrRecsList ) ); // Fill in the new profile list
  if ( pNewItem == NULL )
    return (ZMemError);
  ...
  if ( attrList == NULL ) // Find spot in list
  {
    attrList = pNewItem;
  }
  else
  {
    pLoop = attrList; // Look for end of list
    while ( pLoop->next != NULL )
      pLoop = pLoop->next;
                           ...
                           uint16 cmdFormatLen, uint8 *cmdFormat )
{
  ...
  if ((epDesc->simpleDesc == NULL) ||
      (zcl_DeviceOperational(srcEP, clusterID, hdr.fc.type, cmd, epDesc->simpleDesc->AppProfId) == FALSE))
    return ( ZFailure ); // EMBEDDED RETURN
  
  if ( manuCode != 0 ) // Fill in the Maufacturer Code
  {
    hdr.fc.manuSpecific = 1;
    hdr.manuCode = manuCode;
  }
  
  if ( direction ) // Set the Command Direction
    hdr.fc.direction = ZCL_FRAME_SERVER_CLIENT_DIR;
  else
    hdr.fc.direction = ZCL_FRAME_CLIENT_SERVER_DIR;

  if ( disableDefaultRsp ) // Set the Disable Default Response field
    hdr.fc.disableDefaultRsp = 1;
  else
    hdr.fc.disableDefaultRsp = 0;
                     
  hdr.transSeqNum = seqNum; // Fill in the Transaction Sequence Number
  
  hdr.commandID = cmd; // Fill in the command
  
  msgLen = zclCalcHdrSize( &hdr ); // calculate the needed buffer size
  msgLen += cmdFormatLen;

  msgBuf = osal_mem_alloc( msgLen ); // Allocate the buffer needed
  if ( msgBuf != NULL )
  {
    pBuf = zclBuildHdr( &hdr, msgBuf ); // Fill in the ZCL Header

    osal_memcpy( pBuf, cmdFormat, cmdFormatLen ); // Fill in the command frame
                        ...
                        uint8 direction, uint8 disableDefaultRsp, uint8 seqNum)
{
  ...
  if ( buf != NULL )
  {
    ...
    pBuf = buf; // Load the buffer - serially
...
void zclProcessMessageMSG( afIncomingMSGPacket_t *pkt )
{
  ...
  byte recv_data[20]; // receive data array
  uint16 len;
#endif
  
#if defined(End_Device) // The End Device receive data
  byte receive[20];
  uint16 len;
#endif  
  ...
  if ( pkt->cmd.DataLength == 0 ) return;   // Error, ignore the message
...
/******************** Router / End Device *******************************/
// The device receive the uart command with RF
  ...
  if(pkt->cmd.DataLength > 0)
  {
    for(len = 0; len < pkt->cmd.DataLength; len++)
    {
      ...
      #if defined ( LCD_SUPPORTED )
        HalLcdWriteChar(HAL_LCD_LINE_4, len, receive[len]);
      #endif
    }
...
static void *zclParseInReadRspCmd( zclParseCmd_t *pCmd )
{
  ...
  while ( pBuf < ( pCmd->pData + pCmd->dataLen ) )
  {
    ...
    if ( status == ZCL_STATUS_SUCCESS )
    {
      ...
      if ( PADDING_NEEDED( attrDataLen ) ) // add padding if needed
        attrDataLen++;
      dataLen += attrDataLen;
    }
  }
 
  hdrLen = sizeof( zclReadRspCmd_t ) + ( numAttr * sizeof( zclReadRspStatus_t ) ); // calculate the length of the response header
  ...
  if ( readRspCmd != NULL )
  {
    ...
    for ( i = 0; i < numAttr; i++ )
    {
      ...
      if ( statusRec->status == ZCL_STATUS_SUCCESS )
      {
        ...
        if ( PADDING_NEEDED( attrDataLen ) ) // advance attribute data pointer
          attrDataLen++;
        dataPtr += attrDataLen;
      }
    }
...
void *zclParseInWriteCmd( zclParseCmd_t *pCmd )
{
  ...
  while ( pBuf < ( pCmd->pData + pCmd->dataLen ) ) // find out the number of attributes and the length of attribute data
  {
    numAttr++;
  ...
  hdrLen = sizeof( zclWriteCmd_t ) + ( numAttr * sizeof( zclWriteRec_t ) ); // calculate the length of the response header
...
static void *zclParseInWriteRspCmd( zclParseCmd_t *pCmd )
{
  ...
  if ( writeRspCmd != NULL )
  {
    if ( pCmd->dataLen == 1 )
    {
      writeRspCmd->attrList[i++].status = *pBuf; // special case when all writes were successfull
    }
    else
    {
      while ( pBuf < ( pCmd->pData + pCmd->dataLen ) )
...
void *zclParseInConfigReportCmd( zclParseCmd_t *pCmd )
{
  ...
  while ( pBuf < ( pCmd->pData + pCmd->dataLen ) ) // Calculate the length of the Request command
  {
    numAttr++;
    ...
    if ( direction == ZCL_SEND_ATTR_REPORTS ) // Is there a Reportable Change field?
    {
      dataType = *pBuf++;
      pBuf += 4; // move pass the Min and Max Reporting Intervals
 
      if ( zclAnalogDataType( dataType ) ) // For attributes of 'discrete' data types this field is omitted
      {
        reportChangeLen = zclGetDataTypeLength( dataType );
        pBuf += reportChangeLen;
        
        if ( PADDING_NEEDED( reportChangeLen ) ) // add padding if needed
          reportChangeLen++;
        dataLen += reportChangeLen;
      }
    }
  ...
  { 
    ...
    for ( i = 0; i < numAttr; i++ )
    {
      ...
      if ( reportRec->direction == ZCL_SEND_ATTR_REPORTS )
      {
        ...
        reportRec->dataType = *pBuf++; // Attribute to be reported
        reportRec->minReportInt = BUILD_UINT16( pBuf[0], pBuf[1] );
        pBuf += 2;
        reportRec->maxReportInt = BUILD_UINT16( pBuf[0], pBuf[1] );
        pBuf += 2;
       
        if ( zclAnalogDataType( reportRec->dataType ) ) // For attributes of 'discrete' data types this field is omitted
        {
          ...
          if ( PADDING_NEEDED( reportChangeLen ) ) // advance attribute data pointer
            reportChangeLen++;
      ...
      {
        reportRec->timeoutPeriod = BUILD_UINT16( pBuf[0], pBuf[1] ); // Attribute reports to be received
        pBuf += 2;
      }
    } // while loop
  }
...
static uint8 zclProcessInWriteCmd( zclIncoming_t *pInMsg )
{
  ...
  for (i = 0; i < writeCmd->numAttr; i++)
  {
    ...
    if ( zclFindAttrRec( pInMsg->msg->endPoint, pInMsg->msg->clusterId, statusRec->attrID, &attrRec ) )
    {
      if ( statusRec->dataType == attrRec.attr.dataType )
      {
        ...
        if ( sendRsp && status != ZCL_STATUS_SUCCESS ) // If successful, a write attribute status record shall NOT be generated
        {
          writeRspCmd->attrList[j].status = status; // Attribute is read only - move on to the next write attribute record
          writeRspCmd->attrList[j++].attrID = statusRec->attrID;
        }
      }
      else
      {
        if ( sendRsp ) // Attribute data type is incorrect - move on to the next write attribute record
        {
    ...
    {
      if ( sendRsp ) // Attribute is not supported - move on to the next write attribute record
      {
  ...
  if ( sendRsp )
  {
    ...
    {
      // Since all records were written successful, include a single status record in the resonse command 
      // with the status field set to SUCCESS and the attribute ID field omitted.
 ...
 * @param   pInMsg - incoming message to process
 *          curWriteRec - old data
 *          numAttr - number of attributes to be reverted
 */
static void zclRevertWriteUndividedCmd( zclIncoming_t *pInMsg, zclWriteRec_t *curWriteRec, uint16 numAttr )
{
  zclWriteRec_t *statusRec;
  zclAttrRec_t attrRec;
  uint8 dataLen;
...
static uint8 zclProcessInWriteUndividedCmd( zclIncoming_t *pInMsg )
{
  ...
  writeRspCmd = (zclWriteRspCmd_t *)osal_mem_alloc( sizeof( zclWriteRspCmd_t ) + sizeof( zclWriteRspStatus_t )* writeCmd->numAttr );
