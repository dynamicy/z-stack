...
/* APP */
#if defined(Coor_receiver)||(Router_receiver)
...
/*********************************************************************
 * @fn      osal_strlen
 * @brief   Calculates the length of a string.  The string must be null
 *          terminated.
 ...
 * @brief   This function is used to deallocate a message buffer. This function
...
/*********************************************************************
 * @fn      osal_msg_send
 * @brief   This function is called by a task to send a command message to
 *          another task or processing element.  The sending_task field must
 *          refer to a valid task, since the task ID will be used
 *          for the response message.  This function will also set a message
 *          ready event in the destination tasks event list.
 * @param   uint8 destination task - Send msg to?  Task ID
 *          uint8 *msg_ptr - pointer to new message buffer
 *          uint8 len - length of data in message
...
/*********************************************************************
 * @fn      osal_msg_receive
 * @brief   This function is called by a task to retrieve a received command
 ...
 *              event - The OSAL event id that the enqueued OSAL message must match.
 * output parameters
 * None.
...
void osal_start_system( void )
{
  ...
  #if defined (HAL_KEYPAD)
    ch = halKeypadPushed();
    if(ch > 0)
    { 
    #if defined (End_Device)
      osal_set_event( zclZigbeeDevice_TaskID , KEYPAD_MSG_EVT ); // Set the keypad event
    #endif
    #if ( defined Coor_receiver ) || ( defined Router_receiver )
      osal_set_event( zclZigbeeReceiver_TaskID , KEYPAD_MSG_EVT ); // Set the keypad event
    #endif
    }
  #endif    
...
/*********　The under is task array, check the task have happen ************/
    do 
    {
    ...
    if (idx < tasksCnt)
    {
      ...
      HAL_ENTER_CRITICAL_SECTION(intState); //enter critical section,執行HAL_DISABLE_INTERRUPT();
        events = tasksEvents[idx]; //第idx個任務發生事件, 處理第idx的任務事件
