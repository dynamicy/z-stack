...
void zclSampleSw_Init( byte task_id )
{
  ...
  zclSampleSw_DstAddr.addrMode = (afAddrMode_t)AddrNotPresent; //¶Ç°e¼Ò¦¡-AddrNotPresent=0; AddrBroadcast=15(¼s¼½)
  zclSampleSw_DstAddr.endPoint = 0;                            //¹ï¶HÀ³¥Îµ{¦¡ID
  zclSampleSw_DstAddr.addr.shortAddr = 0;                      //¼s¼½¤º©w¦a§}
...
static void zclSampleSw_HandleKeys( byte shift, byte keys )
{
  ...
  if ( keys & HAL_KEY_SW_2 ) //binding request(¸j©wªº½Ð¨D)
  {
  ...
  if ( keys & HAL_KEY_SW_4 ) //atuo binding(¦Û°Ê¸j©w¸Ë¸m)
  {
    HalLedSet ( HAL_LED_1, HAL_LED_MODE_ON ); //«ö¤UHAL_KEY_SW_4®É,LED_4 off
