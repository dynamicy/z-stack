 ...
 *                                         GLOBAL VARIABLES
 ***************************************************************************************************/
byte App_TaskID; // Used to indentify the application ID for osal task
...
void MT_UartInit ( void )
{
  ...
  App_TaskID = 0; // Initialize APP ID

  /* UART Configuration */
  uartConfig.configured           = TRUE;
  uartConfig.baudRate             = MT_UART_DEFAULT_BAUDRATE;
  uartConfig.flowControl          = FALSE; // FlowControl define RTS, CTS ; TRUE -> Enable, FALSE -> Disable
  ...
  #if defined (Coor_receiver) || defined (Router_receiver)
    uartConfig.callBackFunc         = zclUARTMsg_CallBack; // The Coordinator receive UART command callback
  #endif
  #if defined (End_Device) || defined (Router_Device)
    uartConfig.callBackFunc         = zclRS485_CallBack; // The Coordinator receive UART/RS485 command callback
  #endif
 ...
 *          byte len - length (in bytes) of message
 * @return  result byte
 ***************************************************************************************************/
  ...
  while (Hal_UART_RxBufLen(port))
  {
    ...
    switch (state)
    {
      ...
      case CMD_STATE2:
        ...
        if (LEN_Token) // If there is no data, skip to FCS state
        {
          state = DATA_STATE;
        }
        else
      ...
      case DATA_STATE:
        pMsg->msg[MT_RPC_FRAME_HDR_SZ + tempDataLen++] = ch; // Fill in the buffer the first byte of the data
       
        bytesInRxBuffer = Hal_UART_RxBufLen(port); // Check number of bytes left in the Rx buffer
      ...
      case FCS_STATE:
        ...
        FSC_Token = ch;
        if ((MT_UartCalcFCS ((uint8*)&pMsg->msg[0], MT_RPC_FRAME_HDR_SZ + LEN_Token) == FSC_Token)) // Make sure it's correct
        {
          osal_msg_send( App_TaskID, (byte *)pMsg );
        }
        else
        {
          osal_msg_deallocate ( (uint8 *)pMsg ); // deallocate the msg
        }
        state = SOP_STATE; // Reset the state, send or discard the buffers at this point
        break;
