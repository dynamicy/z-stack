...
UINT16 ZDApp_event_loop( uint8 task_id, UINT16 events )
{
  ...
  if ( ZSTACK_ROUTER_BUILD )
  {
    ...
    {
      ZDApp_NetworkStartEvt();  //The network start event
  ...
  {
    ZDO_UpdateNwkStatus( devState );  // The device state
 ...
 *          options.
 *
 * @return   ZDO_INITDEV_RESTORED_NETWORK_STATE  - The device's network state was
 *           restored.
 *           ZDO_INITDEV_NEW_NETWORK_STATE - The network state was initialized. This could 
 *           mean that ZCD_NV_STARTUP_OPTION said to not restore, or it could mean that 
 *           there was no network state to restore.
 *           ZDO_INITDEV_LEAVE_NOT_STARTED - Before the reset, a network leave was issued
 *           with the rejoin option set to TRUE.  So, the device was notstarted in the 
 *           network (one time only).  The next time this function is called it will start.
 */
...
void ZDApp_NetworkStartEvt( void )
{
  if ( nwkStatus == ZSuccess )
  {
    ...
    osal_pwrmgr_device( PWRMGR_ALWAYS_ON ); // osal device power setting -> ALWAYS ON
    osal_set_event( ZDAppTaskID, ZDO_STATE_CHANGE_EVT ); // TaskID -> ZDO_STATE_CHANGE_EVT
  }
  else
  {
 ...
 *
 * @brief  Sends a Network Join message
 *
...
void ZDApp_SendMsg( uint8 taskID, uint8 cmd, uint8 len, uint8 *buf )
{
  ...
  if ( msgPtr )
  {
    if ( (len > 0) && (buf != NULL) ) osal_memcpy( msgPtr, buf, len );
                                         ...
                                         networkDesc_t *NetworkList )
{
  ...
  for ( stackProfile = 0; stackProfile < STACK_PROFILE_MAX; stackProfile++ )
  {
    ...
    for ( i = 0; i < ResultCount; i++, pNwkDesc = pNwkDesc->nextDesc )
    {
      ...
      // check if beacon order is the right value.. if ( pNwkDesc->beaconOrder < ZDO_CONFIG_MAX_BO )
      // continue;
...
void ZDO_NetworkFormationConfirmCB( ZStatus_t Status )
{
  ...
  if ( Status == ZSUCCESS )
  {
    // LED Blink shows Coordinator started
