...
{
//  Start sending the periodic message in a regular interval. 開始在定期的時間間隔傳送定期訊息
...
//  osal_start_timerEx(ZDAppTaskID, ZDO_NETWORK_INIT, NWK_RETRY_DELAY );   
    ...
    show("Chris:KEY_CHANGE"); 
    
//    sprintf(buf, "Hello, Chris");
//    HalUARTWrite(HAL_UART_PORT_0, buf, 64);
    
    //ZDO_MSG_SEND              
    osal_set_event( ZDO_MSG_SEND_ID, NWK_RETRY_DELAY);
      //  NWK_RETRY_DELAY             1000   // in milliseconds
    osal_start_timerEx( ZDO_MSG_SEND_ID, ZDO_MSG_SEND_EVT, NWK_RETRY_DELAY); //ZDO_MSG_SEND_EVT => AF_DATA_CONFIRM_CMD
}
...
uint16 zclZigbeeSendMsg_event_loop(uint8 task_id, uint16 events)
{
  ...
  {
    MSGpkt = (afIncomingMSGPacket_t *)osal_msg_receive( ZDO_MSG_SEND_ID ); // 透過osal_msg_receive function 接收SampleApp_TaskID標誌
    while ( MSGpkt )
    {    
      switch ( MSGpkt->hdr.event )
      {
        case ZDO_MSG_SEND_EVT: // Received when a key is pressed(當按下key時進行接收)
          ZSendMsg("Hello");
          break;
        default:
          break;
      ...
      MSGpkt = (afIncomingMSGPacket_t *)osal_msg_receive( ZDO_MSG_SEND_ID );
    }
...
//    osal_start_timerEx( ZDO_MSG_SEND_ID, 
//                        ZDO_MSG_SEND_EVT,
//                       (SAMPLEAPP_SEND_PERIODIC_MSG_TIMEOUT + (osal_rand() & 0x00FF)) );

    // return unprocessed events
    return (events ^ ZDO_MSG_SEND_EVT);
  }
...
//        sprintf(temp, "Addr:%d,Data:%s", MSGpkt->macDestAddr,MSGpkt->cmd.Data);
//        show(temp);      
//        sprintf(temp, "Addr:%d", MSGpkt->srcAddr.addr.shortAddr);
//        show(temp);          
        //req.dstAddr.addr.shortAddr
//       sprintf(temp, "Mac:%d,Addr:%d,Data:%s", MSGpkt->macDestAddr, MSGpkt->srcAddr.addr.shortAddr, MSGpkt->cmd.Data);
//        sprintf(temp, "Addr:%d,Data:%s", MSGpkt->srcAddr.addr.shortAddr, MSGpkt->cmd.Data);        
//        show(temp); 
/*
   typedef struct
{
  uint8 *Type;
  uint8 *shortAddr;   //HalUARTWrite(uint8 port, uint8 *buf, uint16 len)
  uint8 *Module;  
  uint8 *Data;
} packet_t;     
 */       
 //       pack.Type = "1";
//        strcpy(pack.shortAddr, MSGpkt->srcAddr.addr.shortAddr);
//        pack.shortAddr = MSGpkt->srcAddr.addr.shortAddr;
//        pack.Data = MSGpkt->cmd.Data;
        
//        show("Type:");
 //       show(itoa(pack.Type));
//uint16  HalUARTWrite(uint8 port, uint8 *buf, uint16 len)        
//        HalUARTWrite ( HAL_UART_PORT_0, pack.Type, 2 );        
//        show("Address:");
//        show(itoa(pack.shortAddr));
//       HalUARTWrite ( HAL_UART_PORT_0, pack.shortAddr, 8 );        
//        show("Data:");     
//       show(pack.Data);
...
/*        typedef struct
        {
            byte Type;
            uint32 shortAddr;
            uint16 Module;  // used for the INTER_PAN feature
            byte  *Data;
        } packet_t;
*/  
/*
        device_manager[0].Type = '0';
        device_manager[0].Module = 140;        
        device_manager[0].shaddr = MSGpkt->srcAddr.addr.shortAddr;
//        device_manager[0].Data = MSGpkt->cmd.Data;
        strcpy(device_manager[0].Data, MSGpkt->cmd.Data);
        
        show("device_manager[0].Type");
        sprintf(temp, "%c", device_manager[0].Type);                  
        show("device_manager[0].Module");        
        sprintf(temp, "%d", device_manager[0].Module);                          
        show("device_manager[0].ShourAddress");        
        sprintf(temp, "%s", device_manager[0].shaddr);                          
        show("device_manager[0].Data");        
        sprintf(temp, "%d", device_manager[0].Data);                                  
 //       sprintf(temp, "shortAddr:%d", MSGpkt->srcAddr.addr.shortAddr);     
        sprintf(temp, "shortAddr:%d", shaddr);          
        show(temp);   
*/        
//        NLME_GetCoordShortAddr();
//        sprintf(temp, "CoordShortAddr:%d", NLME_GetCoordShortAddr());          
//        show(temp);   
