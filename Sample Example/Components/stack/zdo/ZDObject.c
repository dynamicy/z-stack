...
uint32 apsChannelMask = 0;
  ...
  uint8 cnt = 0;
  ZDEndDeviceBind_t dev_sht[5];
  ZDEndDeviceBind_t coordinator;
#endif
 ...
 * @param       numList1 - number of items in list 1, list1 - first list of cluster IDs 
 *              numList2 - number of items in list 2, list2 - second list of cluster IDs
 *              pMatches - buffer to put matches
 ...
 * @param       ACnt  - number of entries in list A, AList  - List A     
 * @param       BCnt  - number of entries in list B, BList  - List B    
 ...
 * @brief   Called to match end device binding requests
 * @param   bindReq  - binding request information
 *          SecurityUse - Security enable/disable
 */
void ZDO_MatchEndDeviceBind( ZDEndDeviceBind_t *bindReq )
{
...
#if defined(Coor_receiver)
  if(((bindReq->srcAddr) != 0) && ((bindReq->srcAddr) != (dev_sht[cnt-1].srcAddr))) // The shortaddr is End Device or Router shortaddr
  {
    ZDO_CopyMatchInfo( &dev_sht[cnt], bindReq );
    cnt++;
  }
  if((bindReq->srcAddr) == 0) // The coordinator shortaddr is number "0"
  {
    ZDO_CopyMatchInfo( &coordinator, bindReq );
  }
#endif
  
  if ( matchED == NULL ) // Is this the first request?
  {
    matchED = (ZDMatchEndDeviceBind_t *)osal_mem_alloc( sizeof ( ZDMatchEndDeviceBind_t ) ); // Create match info structure
    if ( matchED )
    {
      osal_memset( (uint8 *)matchED, 0, sizeof ( ZDMatchEndDeviceBind_t ) ); // Clear the structure
      if ( !ZDO_CopyMatchInfo( &(matchED->ed1), bindReq ) ) // Copy the first request's information
      {
        status = ZDP_NO_ENTRY;
        sendRsp = TRUE;
      }
    ...
    if ( !sendRsp )
    {
      matchED->state = ZDMATCH_WAIT_REQ; // Set into the correct state
      APS_SetEndDeviceBindTimeout( AIB_MaxBindingTime, ZDO_EndDeviceBindMatchTimeoutCB ); // Setup the timeout
    }
  }
   else
    {
  ...
  if ( sendRsp ) // send response to this requester
  {
    ...
    ZDO_RemoveMatchMemory(); // Process ended - release memory used
  }
}
 ...
 *          status - initial message status
 *          TransSeq - next transaction sequence number
 ...
 * @param       inMsg  - incoming message (request)
 *              bindReq - pointer to place to parse message to
 */
 ...
 * @param       inMsg  - incoming message (request)
 *              pReq - place to put parsed information
 */
 ...
 * @param       inMsg - incoming message
 *              pNDRsp - place to parse the message into
 */
